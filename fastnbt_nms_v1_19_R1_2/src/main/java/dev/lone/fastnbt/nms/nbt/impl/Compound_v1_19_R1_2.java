...
@Implementation.CyclicDependency(type = ICompound.class, version = Version.v1_19_R1)
@SuppressWarnings({"unchecked", "unused"})
public class Compound_v1_19_R1_2 implements ICompound<CompoundTag, ListTag, CompoundTag>
{
    @Override
    public CompoundTag newCompoundInstance()
    {
    ...
    {
        if (!handle.contains(key, NbtType.IntArray.id) && handle.contains(key + "Most", 99) && handle.contains(key + "Least", 99))
    ...
    @Override
    public byte[] getByteArray(CompoundTag handle, String key)
    {
        return handle.getByteArray(key);
    }

    @Override
    public int[] getIntArray(CompoundTag handle, String key)
    {
        return handle.getIntArray(key);
    }

    @Override
    public long[] getLongArray(CompoundTag handle, String key)
    {
